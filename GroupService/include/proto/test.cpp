...
int main(){
    ...
    */
    ...
    groupProto::LoginResponse loginres;
    loginres.set_msgid(1);
    loginres.set_errnum(0);
    loginres.set_userid(53);
    loginres.set_name("jixu");
    groupProto::User* user = loginres.add_friends();
    user->set_id(2);
    user->set_name("zou");
    user->set_password("yuhan");
    user->set_state(groupProto::User_STATE_online);
    for(int i = 0; i < 2; ++i){
    ...
    if(loginres.SerializeToString(&send_str)){
        ...
        groupProto::LoginResponse loginres_res;
